...
class Application(tk.Frame):
    def __init__(self, window_width, window_height, master=None):
        ...
        self.canvas_keyboard.bind("<Double-Button-1>", self.double_click_key)
    ...
    def double_click_key(self, event):
        # Retrieve the letter of the clicked key and convert it to uppercase
        clicked_key = self.keyboard.get_key_pressed().upper()

        # Update the content of the first word candidate label
        self.label_word_candidates[0].config(text=clicked_key)
        if clicked_key == 'C':
            self.show_special_popup("This is a command.")
        elif clicked_key == 'Z':
            self.show_special_popup("This is a command.")
        elif clicked_key == 'Y':
            self.show_special_popup("This is a command.")
            ...
            # Default message for other keys
            self.show_default_popup(f"This is not a command.")

    def show_special_popup(self, message):
        popup_window = tk.Toplevel(self.master)
        popup_window.geometry("200x100")
        popup_window.title("Special Key Alert")

        label = tk.Label(popup_window, text=message)
        label.pack()

    def show_default_popup(self, message):
        popup_window = tk.Toplevel(self.master)
        popup_window.geometry("200x100")
        popup_window.title("Uppercase Letter Alert")
